...
from comfy.cli_args import args, PerformanceFeature # ADDED args import here
import torch
import sys
import platform
import weakref
...
def get_torch_device():
    ...
    global cpu_state
    if args.cpu: # ADDED this line: check for --cpu argument first
        return torch.device("cpu") # ADDED this line: force CPU if --cpu is used
    if directml_enabled:
...
def get_total_memory(dev=None, torch_total_too=False):
    ...
    if hasattr(dev, 'type') and (dev.type == 'cpu' or dev.type == 'mps'): # MODIFIED condition to use ==
...
if args.reserve_vram is not None:
    EXTRA_RESERVED_VRAM = args.reserve_vram * 1024 * 1024
...
def unet_manual_cast(weight_dtype, inference_device, supported_dtypes=[torch.float16, torch.bfloat16, torch.float32]):
    if weight_dtype == torch.float32 or weight_dtype == torch.float64: # MODIFIED condition to use ==
        return None
